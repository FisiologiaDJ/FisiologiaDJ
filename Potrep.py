...
with st.expander("💡¿Qué tanto sabes de la Ecuación de Goldman-Hodgkin-Katz?"):
    st.markdown("""
    ¿Sabias que la ecuación de Goldman-Hodgkin-Katz es de gran utilidad en fisiología para calcular el potencial de membrana? La ecuación de Goldman-Hodgkin-Katz describe el potencial de membrana en estado estacionario cuando la membrana es permeable a múltiples iones (sodio (Na⁺), potasio (K⁺), cloro (Cl⁻) y calcio (Ca²⁺)). El voltaje obtenido en la ecuación representa un equilibrio dinámico de los potenciales de equilibrio de cada ion, donde el valor está determinado por el promedio ponderado de los potenciales de equilibrio iónico de cada ion (determinados por los gradientes de concentración) y las respectivas permeabilidades relativas.  
    """)
with st.expander("📖 Instrucciones generales:"):
    st.markdown("""
    1. Elige la concentración intracelular y extracelular para cada ion. 
    2. Si deseas obtener el POTENCIAL DE MEMBRANA selecciona: 🔹***Modo estático***. 
    3. Si deseas simular un POTENCIAL DE ACCIÓN selecciona: 🔹***Modo Dinámico***. 
    """)


st.subheader("🧪Establece las concentraciones iónicas dentro y fuera de la célula (mM)")
...
if mode == "Estático (cálculo del POTENCIAL DE MEMBRANA)":
    ...
    with st.expander("📖 Instrucciones del ***Modo Estático***:"):
        st.markdown("""
        1. Elige la 🌡️temperatura a la que quieres trabajar. 
        2. Establece las 📊permeabilidades de la membrana celular para cada ion.
        3. En la sección de resultados observarás los ⚡POTENCIALES DE EQUILIBRIO para cada ion, el ⚡POTENCIAL DE MEMBRANA, la 📊 contribución de las concentraciones iónicas al valor del potencial de membrana y el 📊 número aproximado de iones que se mueven. 💡Los resultados se actualizan AUTOMÁTICAMENTE.
        """)
    ...
    st.info("💡El Potencial de Membrana está determinado por el promedio ponderado de los Potenciales de Equilibrio Iónico de cada ion y las respectivas permeabilidades relativas.")
    ...
    st.info("💡En esta sección puedes ver la cantidad aproximada de iones que se desplazarían debido a los cambios en la concentración que estableciste al inicio. Se toman como estado inicial los valores establecidos por defecto, así que el ⚡potencial de membrana  de referencia será -74.7 mV y solo se consideran a los cationes predominantes: potasio y sodio.")

    st.markdown(f'<p style="color:red; font-size:18px;"><b>🚀Del potencial inicial ({V_m_initial:.1f} mV) al actual ({V_m:.1f} mV), se movieron aproximadamente {N_ions_int:,} iones de {ion_movido}."</b></p>', unsafe_allow_html=True)

    with st.expander("💡 ANIONES INTRACELULARES NO DIFUSIBLES"):
        st.markdown("""
        Es importante mencionar que en el interior de la célula existen cargas negativas que no pueden salir e incluyen entre otros a proteínas, fosfatos como el ATP y ácidos nucleicos como el ADN, que en conjunto son denominados ANIONES INTRACELULARES NO DIFUSIBLES. En reposo, esta carga negativa esta equilibrada por iones con carga positiva como el potasio (K⁺) y el sodio (Na⁺). Los aniones intracelulares no difusibles son la razón por la que el potasio se mantiene en altas concentraciones en el interior de las células, en condiciones normales el potasio que sale de la célula deja a los aniones intracelulares no difusibles parcialmente descompensados, lo que ocasiona que el interior se vuelva negativo, de esta forma estos participan directamente en el establecimiento del potencial de membrana en reposo.
        """)

    st.info("⚡Dejamos para ti la Ecuación de Goldman-Hodgkin-Katz y cada uno de sus parametros para que puedas recordarla.⚡")
    ...
    st.info("🩺Gracias por visitarnos, puedes emplear esta aplicación gratuitamente y compartirla si así lo deseas. En Fisiología DJ podrás encontrar una gran cantidad de videos e información que te será de utilidad para todos tus cursos. 🙏 Agradecemos infinitamente toda la ayuda brindada por Qwen Studio para el desarrollo e implementación de esta aplicación. Esta aplicación está diseñada solo para fines educativos.")


elif mode == "Dinámico (simulación del POTENCIAL DE ACCIÓN)":

    
    with st.expander("📖 Instrucciones del ***Modo Dinámico:***"):
        st.markdown("""
        1. Elige la temperatura con la que quieres trabajar.
        2. Establece el tiempo que quieres que dure tu simulación.
        3. Elige el tipo de escenario según el numero de iones con los que quieres simular tu potencial de acción.
        4. Para cada ion en la simulacion establece: el tiempo de inicio, el cambio en la permeabilidad (de cerrado a abierto), la duración de este cambio, el cambio en la permeabilidad (de abierto a cerrado) y la duración de este cambio.
        5. Presiona el botón ***INICIAR SIMULACIÓN***: se mostrará una simulación básica de tu potencial de acción, si no te gusta el resultado ajusta los valores nuevamente.
    ...
    t_max = st.slider("Duración total de la simulación (ms)", 10, 300, 50)

    st.subheader("🔹Selecciona cuantos iones quieres emplear en tu simulación🔹")
    ...
    if scenario == "Permeabilidades de sodio y potasio":
        st.info("📊 Puedes emplear este escenario para simular potenciales de acción básicos de 🧠neuronas y células 💪musculares esqueléticas.")
        st.subheader("Sodio")
        t_Na_start = st.slider("Inicio de sodio (ms)", 0, t_max-1, 0, key="t_na_start_sp")
        P_Na_open = st.number_input("Cambio en la permeabilidad para sodio (apertura)", 0.045, 20.0, 1.0, key="p_na_open_sp")
        t_Na_open = st.slider("Duración de la apertura para sodio (ms)", 1, 10, 1, key="t_na_open_sp")
        P_Na_close = st.number_input("Cambio en la permeabilidad para sodio (cierre)", 0.04, P_Na_open, 0.04, key="p_na_close_sp")
        t_Na_close = st.slider("Duración del cierre para sodio (ms)", 1, 10, 1, key="t_na_close_sp")
        ...
        P_K_open = st.number_input("Cambio en la permeabilidad para potasio (apertura)", 1.0, 20.0, 1.0, key="p_k_open_sp")
        t_K_open = st.slider("Duración de la apertura para potasio (ms)", 1, 30, 1, key="t_k_open_sp")
        P_K_close = st.number_input("Cambio en la permeabilidad para potasio (cierre)", 1.0, P_K_open, 1.0, key="p_k_close_sp")
        t_K_close = st.slider("Duración del cierre para potasio (ms)", 1, 100, 1, key="t_k_close_sp")
        ...
        P_Na_open = st.number_input("Cambio en la permeabilidad para sodio (apertura)", 0.045, 20.0, 1.0, key="p_na_open_spc")
        t_Na_open = st.slider("Duración de la apertura para sodio (ms)", 1, 10, 1, key="t_na_open_spc")
        P_Na_close = st.number_input("Cambio en la permeabilidad para sodio (cierre)", 0.04, P_Na_open, 0.04, key="p_na_close_spc")
        t_Na_close = st.slider("Duración del cierre para sodio (ms)", 1, 10, 1, key="t_na_close_spc")
    
        st.subheader("Potasio")
        na_end = t_Na_start + t_Na_open + t_Na_close
        t_K_start = st.slider("Inicio de potasio (ms)", 0, t_max-1, max(1, na_end - 1), key="t_k_start_spc")
        P_K_open = st.number_input("Cambio en la permeabilidad para potasio (apertura)", 1.0, 20.0, 1.0, key="p_k_open_spc")
        t_K_open = st.slider("Duración de la apertura para potasio (ms)", 1, 30, 1, key="t_k_open_spc")
        P_K_close = st.number_input("Cambio en la permeabilidad para potasio (cierre)", 1.0, P_K_open, 1.0, key="p_k_close_spc")
        t_K_close = st.slider("Duración del cierre para potasio (ms)", 1, 100, 1, key="t_k_close_spc")

        st.subheader("Cloro")
        K_end = t_K_start + t_K_open + t_K_close
        t_Cl_start = st.slider("Inicio de cloro (ms)", 0, t_max-1, 0, key="t_cl_start_spc")
        P_Cl_open = st.number_input("Cambio en la permeabilidad para cloro (apertura)", 0.45, 20.0, 1.0, key="p_cl_open_spc")
        t_Cl_open = st.slider("Duración de la apertura para cloro (ms)", 1, 50, 1, key="t_cl_open_spc")
        P_Cl_close = st.number_input("Cambio en la permeabilidad para cloro (cierre)", 0.45, P_Cl_open, 0.45, key="p_cl_close_spc")
        t_Cl_close = st.slider("Duración del cierre para cloro (ms)", 1, 50, 1, key="t_cl_close_spc")
    
    
    elif scenario == "Permeabilidades de sodio, potasio/calcio":
        st.info("📊 Puedes emplear este escenario para simular potenciales de acción básicos de células musculares 🫀cardíacas.")
        st.subheader("Sodio")
        t_Na_start = st.slider("Inicio de sodio (ms)", 0, t_max-1, 0, key="t_Na_start_spca")
        P_Na_open = st.number_input("Cambio en la permeabilidad para sodio (apertura)", 0.045, 20.0, 1.0, key="p_na_open_spca")
        t_Na_open = st.slider("Duración de la apertura para sodio (ms)", 1, 10, 1, key="t_na_open_spca")
        P_Na_close = st.number_input("Cambio en la permeabilidad para sodio (cierre)", 0.04, P_Na_open, 0.04, key="p_na_close_spca")
        t_Na_close = st.slider("Duración del cierre para sodio (ms)", 1, 10, 1, key="t_na_close_spca")
        ...
        P_K_open = st.number_input("Cambio en la permeabilidad para potasio rapido (apertura)", 0.01, 20.0, 1.0, key="p_k_fast_open_spca")
        t_K_open = st.slider("Duración de la apertura para potasio rapido (ms)", 1, 10, 1, key="t_k_fast_open_spca")
        P_K_close = st.number_input("Cambio en la permeabilidad para potasio rapido (cierre)", 0.01, P_K_open, 1.0, key="p_k_fast_close_spca")
        t_K_close = st.slider("Duración del cierre para potasio rapido (ms)", 1, 5, 1, key="t_k_fast_close_spca")
        ...
        P_Ca_open = st.number_input("Cambio en la permeabilidad para Calcio (apertura)", 0.0001, 500.0, 1.0, key="p_ca_open_spca")
        t_Ca_open = st.slider("Duración de la apertura para Calcio (ms)", 1, 100, 1, key="t_ca_open_spca")
        P_Ca_close = st.number_input("Cambio en la permeabilidad para Calcio (cierre)", 0.0001, P_K_open, 0.5, key="p_ca_close_spca")
        t_Ca_close = st.slider("Duración del cierre para Calcio (ms)", 1, 100, 1, key="t_ca_close_spca")
        ...
        P_K_meseta_open = st.number_input("Cambio en la permeabilidad para potasio de la meseta (apertura)", 0.01, 20.0, 1.0, key="p_k_meseta_open_spca")
        t_K_meseta_open = st.slider("Duración de la apertura para potasio de la meseta (ms)", 1, 100, 1, key="t_k_meseta_open_spca")
        P_K_meseta_close = st.number_input("Cambio en la permeabilidad para potasio de la meseta (cierre)", 0.01, P_K_meseta_open, 1.0, key="p_k_meseta_close_spca")
        t_K_meseta_close = st.slider("Duración del cierre para potasio de la meseta (ms)", 1, 50, 1, key="t_k_meseta_close_spca")
        ...
        P_K_slow_open = st.number_input("Cambio en la permeabilidad para potasio lento (apertura)", 0.01, 50.0, 1.0, key="p_k_slow_open_spca")
        t_K_slow_open = st.slider("Duración de la apertura para potasio lento (ms)", 1, 30, 1, key="t_k_slow_open_spca")
        P_K_slow_close = st.number_input("Cambio en la permeabilidad para potasio lento (cierre)", 0.01, P_K_slow_open, 1.0, key="p_k_slow_close_spca")
        t_K_slow_close = st.slider("Duración del cierre para potasio lento (ms)", 1, 30, 1, key="t_k_slow_close_spca")


    elif scenario == "Permeabilidades de sodio/potasio y calcio":
        st.info("📊 Puedes emplear este escenario para simular potenciales de acción básicos de células marcapaso 🫀cardíacas.")
        st.subheader("Sodio (If)")
        t_Na_start = st.slider("Inicio de sodio (ms)", 0, t_max-1, 0, key="t_na_start_spcal")
        P_Na_open = st.number_input("Cambio en la permeabilidad de sodio (apertura)", 0.1, 20.0, 0.1, key="p_na_open_spcal")
        t_Na_open = st.slider("Duración de la apertura para sodio (ms)", 1, 100, 1, key="t_na_open_spcal")
        P_Na_close = st.number_input("Cambio en la permeabilidad para sodio (cierre)", 0.04, P_Na_open, 0.04, key="p_na_close_spcal")
        t_Na_close = st.slider("Duración del cierre para sodio (ms)", 1, 100, 1, key="t_na_close_spcal")

        st.subheader("Potasio (If)")
        t_K_if_start = st.slider("Inicio de potasio (ms)", 0, t_max-1, t_Na_start, key="t_k_if_start_spcal")
        P_K_if_open = st.number_input("Cambio en la permeabilidad para potasio (apertura)", 0.01, 20.0, 1.0, key="p_k_if_open_spcal")
        t_K_if_open = st.slider("Duración de la apertura para potasio (ms)", 1, 100, 1, key="t_k_if_open_spcal")
        P_K_if_close = st.number_input("Cambio en la permeabilidad para potasio (cierre)", 0.01, P_K_if_open, 1.0, key="p_k_if_close_spcal")
        t_K_if_close = st.slider("Duración del cierre para potasio (ms)", 1, 100, 1, key="t_k_if_close_spcal")

        st.subheader("Calcio")
        Na_end = t_Na_start + t_Na_open + t_Na_close
        t_Ca_start = st.slider("Inicio de Calcio (ms)", 0, t_max-1, max(1, Na_end - 1), key="t_ca_start_spcal")
        P_Ca_open = st.number_input("Cambio en la permeabilidad para Calcio (apertura)", 0.0001, 500.0, 1.0, key="p_ca_open_spcal")
        t_Ca_open = st.slider("Duración de la apertura para Calcio (ms)", 1, 100, 1, key="t_ca_open_spcal")
        P_Ca_close = st.number_input("Cambio en la permeabilidad para Calcio (cierre)", 0.0001, P_K_open, 1.0, key="p_ca_close_spcal")
        t_Ca_close = st.slider("Duración del cierre para Calcio (ms)", 1, 100, 1, key="t_ca_close_spcal")
        ...
        P_K_rif_open = st.number_input("Cambio en la permeabilidad para potasio (apertura)", 0.01, 20.0, 1.0, key="p_k_rif_open_spcal")
        t_K_rif_open = st.slider("Duración de la apertura para potasio (ms)", 1, 100, 1, key="t_k_rif_open_spcal")
        P_K_rif_close = st.number_input("Cambio en la permeabilidad para potasio (cierre)", 0.01, P_K_open, 1.0, key="p_k_rif_close_spcal")
        t_K_rif_close = st.slider("Duración del cierre para potasio (ms)", 1, 100, 1, key="t_k_rif_close_spcal")
